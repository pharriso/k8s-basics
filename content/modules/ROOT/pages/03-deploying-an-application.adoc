...
As you are running on an environment provisioned specifically for the workshop, it does mean that we have to make a few changes to the application deployment before we can go ahead and deploy. 
...
image::terminal-window.png[Parksmap Ingress, 900]
...
[source,execute]
----
curl http://nationalparks-py.wksp-{user}.svc.cluster.local:8080/ws/data/all
----
