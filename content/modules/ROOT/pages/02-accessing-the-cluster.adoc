...
*Note from Phil....The login part below needs to change, it will not work using the workshop framework as is. Need to come up with a way the attendee can login using kubectl. I've tried and failed so far as the cluster is using keycloak for login (I think)*
*Worst caswe, we can put some instructions in to go to the console and get the login command using a token and oct command. Comming back to it later*

*For now, you are OCP experts, just go to the console, get the login command and token and use that for testing*
...
As an enterprise Kubernetes deployment, the environment you are using is locked down and you are unable to create namespaces. Namespaces enable you to isolate your resources within a single cluster. The isolation is what's enabling all workshop attendees to perform the same labs on the same cluster without impacting each other's work.
...
For reference, you can see an example of the terminal window on the right hand side of the browser below.
